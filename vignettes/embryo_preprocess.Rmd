...
We will demonstrate a comparative analysis using data from a study of the early mouse embryo.
E8.5 embryos were dissociated and used to generate scRNA-seq data from the 10X Genomics protocol [@zheng2017massively].
This was performed using embryos from both wild-type (B6) mice and _Tal1_-knockout mice.
...
For each genotype, two biological replicates are available where each replicate contains over 10,000 cells from an independent pool of 6-7 embryos.
...
counts <- as(counts, "dgCMatrix") # more space-efficient than dgTMatrix.
dim(counts)
```
...
We then store the count matrix in a `SingleCellExperiment` object with the metadata associated with each cell and gene.
...
gene.tab <- read.delim(file.path(tempdir(), gene.path), header=FALSE)
colnames(gene.tab) <- c("ENSEMBL", "SYMBOL")

library(SingleCellExperiment)
sce <- SingleCellExperiment(list(counts=counts), colData=meta.tab, rowData=gene.tab)
sce
```

This object contains cells from all four samples, with positive staining for the Tomato marker indicating that _Tal1_ was knocked out.
...
plot(colSums(counts(sce)), sizeFactors(sce), xlab="Total library size", 
